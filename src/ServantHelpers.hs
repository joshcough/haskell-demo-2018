...
adminOr401 u = whenWithNoAuthErr (userEmail u == "joshcough@gmail.com")

-- |
callerIsUserOr401 :: MonadError FileServerDemoError m => User -> DbUserId -> m a -> m a
callerIsUserOr401 caller uid = whenWithNoAuthErr (userId caller == fromSqlKey uid)

-- |
callerIsUserOrIsAdminElse401 :: MonadError FileServerDemoError m => User -> DbUserId -> m a -> m a
callerIsUserOrIsAdminElse401 caller uid =
    whenWithNoAuthErr (userEmail caller == "joshcough@gmail.com" || userId caller == fromSqlKey uid)

-- |
whenWithNoAuthErr :: MonadError (AppError e) m => Bool -> m a -> m a
whenWithNoAuthErr b m = if b then m else throwError (AppAuthError NoAuthError)